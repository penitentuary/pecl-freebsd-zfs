...
/* Forward declaration for recursive iteration */
static int zfs_list_callback(zfs_handle_t *zhp, void *data);

/* Callback function for iterating datasets */
static int zfs_list_callback(zfs_handle_t *zhp, void *data)
{
    ...
    /* Recurse into child datasets before closing the handle */
    zfs_iter_children(zhp, zfs_list_callback, data);

    zfs_close(zhp);
    return 0;
}

/* Main function to list ZFS datasets */
int zfs_list_datasets(libzfs_handle_t *g_zfs, zval *return_value)
{
     ...
     * zfs_iter_root() gives us pool root datasets, then the callback
     * recurses via zfs_iter_children() to enumerate the full tree.
     */
    ret = zfs_iter_root(g_zfs, zfs_list_callback, &cb_data);

    if (ret != 0) {
        php_zfs_error("Failed to iterate ZFS datasets: %s",
